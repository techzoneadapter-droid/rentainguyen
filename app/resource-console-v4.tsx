...
type RunKind =
  | 'health' | 'sync' | 'crm' | 'open' | 'refresh' | 'billing' | 'campaigns'
  | 'renameAd' | 'spendCap' | 'renameBusiness' | 'inviteUser' | 'backup' | 'claimPage'
  | 'systemUser' | 'pageRead' | 'pagePost' | 'pagePublish' | 'pageUnpublish' | 'pageInfo' | 'unsupported';
...
type Billing = { id: string; name: string; accountStatus: number; disableReason: number; currency: string; balance: string; amountSpent: string; spendCap: string; hasFundingSource: boolean; fundingType: string; fundingDisplay: string };
type ActionItem = { id: string; title: string; kind: RunKind; icon?: string; danger?: boolean; note?: string };

const STATUS_TEXT: Record<TokenStatus, string> = {
  active: 'LIVE', invalid: 'DIE', permission_issue: 'API từ chối', rate_limited: 'Rate limit', create_restricted: 'Giới hạn tạo', unknown_error: 'Chưa kiểm tra',
};
...
const ADS_TOOLS: ActionItem[] = [
  ...
  { id: 'prepay', title: 'Kích hoạt trả trước', kind: 'unsupported', icon: 'lock' },
  { id: 'add-card', title: 'Add thẻ', kind: 'unsupported', icon: 'lock' },
  { id: 'add-user', title: 'Thêm người', kind: 'unsupported', icon: 'lock' },
  { id: 'delete-draft', title: 'Xóa Camp, Nháp', kind: 'unsupported', icon: 'lock' },
  { id: 'invoice', title: 'Tải hóa đơn', kind: 'unsupported', icon: 'lock' },
  { id: 'leave', title: 'Thoát tài khoản', kind: 'unsupported', icon: 'lock' },
  { id: 'share-partner', title: 'Share đối tác BM', kind: 'unsupported', icon: 'lock' },
  { id: 'share-pixel', title: 'Share Pixel', kind: 'unsupported', icon: 'lock' },
  { id: 'phone', title: 'Xác minh phone', kind: 'unsupported', icon: 'lock' },
];
...
const BM_TOOLS: ActionItem[] = [
  ...
  { id: 'crm', title: 'Đẩy CRM', kind: 'crm' },
  { id: 'bm3', title: 'Kích BM3', kind: 'unsupported', icon: 'lock' },
  { id: 'card', title: 'Add thẻ', kind: 'unsupported', icon: 'lock' },
  { id: 'asset-group', title: 'Nhóm tài sản BM', kind: 'unsupported', icon: 'lock' },
  { id: 'cancel-invite', title: 'Hủy lời mời', kind: 'unsupported', icon: 'lock' },
  { id: 'remove-asset', title: 'Xóa tài sản khỏi BM', kind: 'unsupported', icon: 'lock' },
  { id: 'partner', title: 'Xóa đối tác', kind: 'unsupported', icon: 'lock' },
  { id: 'remove-ad', title: 'Xóa TKQC', kind: 'unsupported', icon: 'lock' },
  { id: 'permissions', title: 'Cập nhật quyền', kind: 'unsupported', icon: 'lock' },
];
...
const PAGE_TOOLS: ActionItem[] = [
  ...
  { id: 'crm', title: 'Đẩy CRM', kind: 'crm' },
  { id: 'partner', title: 'Thêm đối tác', kind: 'unsupported', icon: 'lock' },
  { id: 'add-bm', title: 'Add Page vào BM', kind: 'unsupported', icon: 'lock' },
  { id: 'assign-bm', title: 'Chỉ định Page BM', kind: 'unsupported', icon: 'lock' },
  { id: 'avatar', title: 'Đổi Avatar & Cover', kind: 'unsupported', icon: 'lock' },
  { id: 'delete-posts', title: 'Xóa bài viết hàng loạt', kind: 'unsupported', icon: 'lock' },
  { id: 'leave', title: 'Thoát Page', kind: 'unsupported', icon: 'lock' },
  { id: 'remove-admin', title: 'Xóa QTV Page', kind: 'unsupported', icon: 'lock' },
  { id: 'rename', title: 'Đổi tên Page', kind: 'unsupported', icon: 'lock' },
  { id: 'share', title: 'Share Page', kind: 'unsupported', icon: 'lock' },
];
...
function cleanTokenCandidate(value: unknown) {
  return String(value ?? '').replace(/^\s*Bearer\s+/i, '').replace(/[\u200B-\u200D\uFEFF]/g, '').replace(/^['"]|['"]$/g, '').replace(/\s+/g, '').trim();
}
...
export default function ResourceConsoleV4() {
  ...
  async function jsonFetch<T>(url: string, init?: RequestInit) {
    ...
    const data = await response.json() as T & { error?: string; message?: string; unsupported?: boolean };
  ...
  async function syncToken(tokenId: string) {
    const data = await jsonFetch<{ imported?: number }>('/api/resource-create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'import_token', tokenId }) });
  ...
  async function importCheckAndSync() {
    ...
    try {
      ...
      for (let index = 0; index < items.length; index += 20) {
        ...
        const data = await jsonFetch<{ inventories?: Inventory[]; imported?: number; reused?: number }>('/api/token-runtime', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'import', items: chunk }) });
  ...
  async function scanTokens(ids: string[]) {
    ...
    try {
      for (let i = 0; i < ids.length; i += 20) {
        ...
        const data = await jsonFetch<{ inventories?: Inventory[] }>('/api/token-runtime', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'scan', ids: chunk }) });
  ...
  async function renameToken(token: TokenRow) {
    ...
    try {
      const data = await jsonFetch<{ tokens?: TokenRow[]; message?: string }>('/api/token-runtime', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'rename', id: token.id, label }) });
  ...
  async function deleteToken(token: TokenRow) {
    ...
    setBusy(true); setError('');
    try {
      const data = await jsonFetch<{ tokens?: TokenRow[]; message?: string }>('/api/token-runtime', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'delete', ids: [token.id], purgeAssets: true }) });
  ...
  async function health(ids: string[]) {
    ...
    setBusy(true); setError('');
    try {
      const data = await jsonFetch<{ message?: string }>('/api/resource-health', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids, tokenId: selectedToken || '' }) });
  ...
  async function pushCrm(ids: string[]) {
    ...
    try {
      ...
      for (let i = 0; i < metaIds.length; i += 100) {
        const data = await jsonFetch<{ accepted?: unknown[] }>('/api/crm-push', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ metaIds: metaIds.slice(i, i + 100) }) });
  ...
  async function runResourceAction(kind: RunKind, title: string) {
    ...
    const call = async (action: string, payload: Record<string, unknown> = {}) => {
      const data = await jsonFetch<{ ok?: boolean; unsupported?: boolean; message?: string; url?: string }>('/api/resource-actions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tokenId, assetId: selectedAsset.id, action, payload }) });
    ...
    setBusy(true); setError(''); setMessage('');
    try {
      if (kind === 'open') await call('open_official');
      else if (kind === 'refresh') await call('refresh');
      else if (kind === 'billing') await readBilling();
      else if (kind === 'campaigns') await loadCampaigns();
      else if (kind === 'renameAd') { const name = window.prompt('Tên TKQC mới:', selectedAsset.name)?.trim(); if (name) await call('rename_ad_account', { name }); }
      else if (kind === 'spendCap') { const spendCap = window.prompt('Giới hạn chi tiêu theo đơn vị API Meta, ví dụ 50000:', '')?.trim(); if (spendCap) await call('set_spend_cap', { spendCap }); }
      else if (kind === 'renameBusiness') { const name = window.prompt('Tên BM mới:', selectedAsset.name)?.trim(); if (name) await call('rename_business', { name }); }
      else if (kind === 'inviteUser') await inviteBusinessUser();
      else if (kind === 'backup') backupBm();
      else if (kind === 'claimPage') { const pageId = window.prompt('Page ID cần thêm vào BM:', '')?.trim(); if (pageId) await call('claim_page', { pageId }); }
      else if (kind === 'systemUser') { const name = window.prompt('Tên System User:', 'System User')?.trim(); if (name) await call('create_system_user', { name, role: 'EMPLOYEE' }); }
      else if (kind === 'pageRead') await loadPageData();
      else if (kind === 'pagePost') await publishPagePost();
      else if (kind === 'pagePublish') await call('publish_state', { state: 'true' });
      else if (kind === 'pageUnpublish') { if (window.confirm('Hủy đăng Page này?')) await call('publish_state', { state: 'false' }); }
      else if (kind === 'pageInfo') { const about = window.prompt('About/giới thiệu mới:', '')?.trim(); const website = window.prompt('Website mới, bỏ trống nếu không đổi:', '')?.trim(); await call('update_page_info', { about, website }); }
      else await call('unsupported', { title });
    } catch (err) { setError((err as Error).message); }
    finally { setBusy(false); }
  }
  ...
  async function createBm() {
    ...
    setBusy(true); setError('');
    try {
      const data = await jsonFetch<{ message?: string }>('/api/business-manager', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tokenId: selectedToken, name: bmName, primaryPage: bmPage, timezone: Number(bmTimezone), vertical: bmVertical, adminEmail: '', purposeConfirmed: true }) });
  ...
  async function inviteBusinessUser() {
    ...
    const data = await jsonFetch<{ message?: string }>('/api/meta-token-ops', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'invite_business_user', tokenId: selectedAsset.sourceTokenId || selectedToken, businessId: metaId(selectedAsset), email, role, purposeConfirmed: true }) });
    setMessage(data.message || 'Đã gửi lời mời.');
  }
  async function readBilling() {
    if (!selectedAsset || selectedAsset.type !== 'TKQC') return;
    const data = await jsonFetch<{ billing?: Billing }>('/api/meta-token-ops', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'read_billing', tokenId: selectedAsset.sourceTokenId || selectedToken, adAccountId: metaId(selectedAsset) }) });
  ...
  async function changeCampaign(campaignId: string, status: 'ACTIVE' | 'PAUSED' | 'DELETED') {
    ...
    const data = await jsonFetch<{ message?: string }>('/api/campaign-ops', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tokenId: selectedAsset.sourceTokenId || selectedToken, accountId: metaId(selectedAsset), campaignId, status }) });
    setMessage(data.message || 'Đã cập nhật campaign.'); await loadCampaigns();
  }
  async function loadPageData() {
    if (!selectedAsset || selectedAsset.type !== 'Page') return;
  ...
  async function publishPagePost() {
    ...
    const data = await jsonFetch<{ message?: string }>('/api/page-ops', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'publish', tokenId: selectedAsset.sourceTokenId || selectedToken, pageId: metaId(selectedAsset), message: messageText }) });
  ...
  async function deletePagePost(postId: string) {
    ...
    const data = await jsonFetch<{ message?: string }>('/api/page-ops', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'delete_post', tokenId: selectedAsset.sourceTokenId || selectedToken, pageId: metaId(selectedAsset), postId }) });
  ...
  function currentTools() { return resourceTab === 'BM' ? BM_TOOLS : resourceTab === 'TKQC' ? ADS_TOOLS : PAGE_TOOLS; }
  function alerts() { return <>{progress && <div className={styles.progress}><LoaderCircle className={styles.spin} size={14}/>{progress}</div>}{error && <div className={styles.error}>{error}<button onClick={()=>setError('')}><X size={14}/></button></div>}{message && <div className={styles.success}><CheckCircle2 size={14}/>{message}<button onClick={()=>setMessage('')}><X size={14}/></button></div>}</>; }
  function renderToolPanel() {
    ...
    return <aside style={{ background:'#fff', border:'1px solid #e0e6ef', borderRadius:14, minHeight:520, maxHeight:'calc(100vh - 178px)', overflow:'auto', padding:14 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}><div><strong>Bảng công cụ {RESOURCE_META[resourceTab].label}</strong><div style={{ fontSize:10, color:'#8b94a6' }}>{selectedAsset ? selectedAsset.name : `${bulkIds.length} tài nguyên đang lọc`}</div></div><Settings2 size={16}/></div>
      <div style={{ display:'grid', gap:8 }}>{currentTools().map((tool) => <button key={tool.id} className={styles.ghost} onClick={() => void runResourceAction(tool.kind, tool.title)} disabled={busy || (!selectedAsset && !['health','sync','crm'].includes(tool.kind))} style={{ justifyContent:'space-between', opacity: tool.icon === 'lock' ? 0.72 : undefined }}><span style={{ display:'flex', alignItems:'center', gap:8 }}>{tool.icon === 'lock' ? <Lock size={13}/> : <Sparkles size={13}/>} {tool.title}</span><ChevronRight size={13}/></button>)}</div>
  ...
  if (mode === 'home') return <div className={styles.shell}><TopBar mode={mode} live={liveTokens.length} total={tokens.length} onMode={(next)=>void openMode(next)}/><main className={styles.home}><section className={styles.hero}><div className={styles.kicker}>ADS WORKSPACE</div><h1>Token → tài nguyên → bảng công cụ thật</h1><p>Không còn nút trống: các chức năng có API sẽ chạy bằng token nguồn; chức năng cần cookie/private endpoint/Billing nhạy cảm sẽ báo rõ không hỗ trợ token-only.</p><div className={styles.heroActions}><button onClick={()=>void openMode('create')}><Plus size={15}/> Nạp token</button><button className={styles.secondary} onClick={()=>void openMode('manage')}><ShieldCheck size={15}/> Quản lý tài nguyên</button></div></section><div className={styles.flowGrid}><article><KeyRound size={18}/><strong>1. Check token</strong><span>Không đánh DIE cho lỗi API-context như Error loading application/Invalid request nếu Meta không báo hết hạn rõ ràng.</span></article><article><Building2 size={18}/><strong>2. BM/ADS/PAGE</strong><span>Tài nguyên được gắn token nguồn để thao tác đúng quyền.</span></article><article><Sparkles size={18}/><strong>3. Công cụ mẫu</strong><span>Bảng công cụ theo từng mục ADS/BM/PAGE giống mẫu, có backend xử lý phía sau.</span></article></div></main></div>;
