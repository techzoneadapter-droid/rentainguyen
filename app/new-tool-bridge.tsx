...
const tools: ToolInfo[] = [
  { key: 'tokens', mode: 'manage', label: 'Nạp & phân loại token', description: 'Import token, check LIVE/DIE, quyền và toàn bộ tài nguyên token.', icon: KeyRound },
  { key: 'resources', mode: 'manage', label: 'Trung tâm tài nguyên', description: 'Quản lý BM, TKQC, Page, Pixel/Dataset đã đồng bộ từ token.', icon: Database },
  { key: 'health', mode: 'manage', label: 'Health Check', description: 'Kiểm tra trạng thái BM, TKQC, Page và Pixel bằng token đã lưu.', icon: HeartPulse },
  { key: 'campaigns', mode: 'manage', label: 'Quản lý Camp', description: 'Đọc campaign theo TKQC và bật/tạm dừng bằng Meta Graph API.', icon: Megaphone, native: true },
  { key: 'workflow', mode: 'manage', label: 'Workflow', description: 'Giữ nguyên workflow runner và dữ liệu vận hành hiện có.', icon: WorkflowIcon },
  { key: 'crm', mode: 'manage', label: 'Đẩy sang CRM', description: 'Giữ nguyên luồng đẩy tài nguyên sang CRM.', icon: Send },
  ...
  { key: 'bm-create', mode: 'create', label: 'Tạo BM từ token', description: 'Preflight quyền, chọn Page đại diện, timezone, vertical và tạo BM thật.', icon: Building2 },
  { key: 'queue', mode: 'create', label: 'Hàng đợi tạo BM', description: 'Tạo danh sách BM, pause/resume/retry và theo dõi tiến trình.', icon: Factory },
...
function parseTokenText(input: string) {
  ...
  input.split(/\r?\n/).forEach((rawLine) => {
    ...
    if (urlToken) push(decodeURIComponent(urlToken[1]));
...
function CampaignManager({
  ...
  useEffect(() => {
    if (!accountId && adAccounts[0]) setAccountId(metaId(adAccounts[0]));
  }, [accountId, adAccounts]);

  async function loadCampaigns() {
    if (!selectedToken || !accountId) {
  ...
  async function updateStatus(campaign: Campaign, status: 'ACTIVE' | 'PAUSED') {
    if (!selectedToken || !accountId) return;
    ...
    try {
      const response = await fetch('/api/ez-ads', {
        ...
        body: JSON.stringify({ tokenId: selectedToken, accountId, campaignId: campaign.id, status }),
      });
      const data = await response.json() as { error?: string; message?: string };
      if (!response.ok) throw new Error(data.error || 'Không cập nhật được campaign.');
      setMessage(data.message || `Đã chuyển ${campaign.name} sang ${status}.`);
      await loadCampaigns();
    } catch (err) {
      setError((err as Error).message);
    } finally {
  ...
  return (
    <div className={styles.nativeToolPage}>
      <div className={styles.pageHeading}>
        <div>
          <span className={styles.eyebrow}>META GRAPH API</span>
          <h1>Quản lý Camp</h1>
          <p>Đọc campaign của TKQC đã đồng bộ và bật/tạm dừng bằng token bạn đã nạp.</p>
        </div>
        <button className={styles.primaryButton} type="button" onClick={() => void loadCampaigns()} disabled={loading || !selectedToken || !accountId}>
          {loading ? <LoaderCircle className={styles.spin} size={16} /> : <RefreshCw size={16} />} Tải campaign
        </button>
      </div>
      ...
      <div className={styles.campaignControls}>
        <label>Token
          <select value={selectedToken} onChange={(event) => setSelectedToken(event.target.value)}>
            ...
            {tokens.map((token) => <option key={token.id} value={token.id}>{token.label} · {statusLabel[token.status]}</option>)}
          </select>
        </label>
        <label>TKQC
          <select value={accountId} onChange={(event) => setAccountId(event.target.value)}>
            <option value="">Chọn TKQC</option>
            {adAccounts.map((asset) => <option key={asset.id} value={metaId(asset)}>{asset.name} · {metaId(asset)}</option>)}
          </select>
        </label>
        <label className={styles.searchField}>Tìm campaign
          <span><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tên / ID / trạng thái" /></span>
        </label>
      </div>
      ...
      <div className={styles.campaignTable}>
        ...
        {filtered.length === 0 ? (
          <div className={styles.emptyCampaign}>Chưa có dữ liệu campaign. Chọn token + TKQC rồi bấm “Tải campaign”.</div>
          ...
          <div className={styles.campaignRow} key={campaign.id}>
            ...
            <span>{campaign.effectiveStatus || '—'}</span>
            ...
            <span className={styles.rowActions}>
              <button type="button" disabled={busyId === campaign.id || campaign.status === 'ACTIVE'} onClick={() => void updateStatus(campaign, 'ACTIVE')}>Bật</button>
              <button type="button" disabled={busyId === campaign.id || campaign.status === 'PAUSED'} onClick={() => void updateStatus(campaign, 'PAUSED')}>Tạm dừng</button>
            </span>
          </div>
        ))}
      </div>
...
export default function NewToolBridge() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      ].filter((button) => button.dataset.bridgeAnchor !== '1' && button.dataset.bridgeAutoClicked !== '1');
  ...
  useEffect(() => {
    const routeClick = (event: MouseEvent) => {
      ...
      const mapping: Record<string, { mode: Mode; tool: ToolKey }> = {
        'Phân loại token': { mode: 'manage', tool: 'tokens' },
        'Trung tâm tài nguyên': { mode: 'manage', tool: 'resources' },
        'Workflow': { mode: 'manage', tool: 'workflow' },
        'Quản lý Camp': { mode: 'manage', tool: 'campaigns' },
        'Quản lý TKQC': { mode: 'manage', tool: 'resources' },
        'Quản lý Pixel': { mode: 'manage', tool: 'resources' },
        'Quản lý BM': { mode: 'manage', tool: 'resources' },
        'Quản lý Page': { mode: 'manage', tool: 'resources' },
        'Tạo BM': { mode: 'create', tool: 'bm-create' },
      };
  ...
  async function quickImportAndSync() {
    ...
    if (!items.length) {
      setError('Dán token hoặc chọn file token trước.');
      return;
    }
    if (items.length > 200) {
      setError('Mỗi lượt tối đa 200 token để hạn chế rate limit.');
      return;
    }
    setBusy(true);
    setError('');
    ...
    try {
      for (let index = 0; index < items.length; index += 20) {
        ...
        (data.inventories || []).filter((item) => item.status === 'active').forEach((item) => liveIds.add(item.tokenId));
      }
      ...
      for (let index = 0; index < ids.length; index += 1) {
        ...
        try {
          ...
          syncedTokens += 1;
          importedResources += data.imported || 0;
        } catch (err) {
      ...
      setFileName('');
      await loadOverview();
      setMessage(`Đã xử lý ${items.length} token · mới ${imported} · có sẵn ${reused} · LIVE ${ids.length} · đồng bộ ${syncedTokens} token / ${importedResources} tài nguyên.${syncErrors.length ? ` ${syncErrors.length} token chưa đồng bộ được; mở Kho token để xem quyền/lỗi.` : ''}`);
    } catch (err) {
      setError((err as Error).message);
    } finally {
      setProgress('');
  ...
  async function syncSelectedToken() {
    ...
    setError('');
    setMessage('');
    setProgress('Đang đồng bộ tài nguyên từ token đã chọn…');
    try {
      const response = await fetch('/api/resource-create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) throw new Error(data.error || 'Không đồng bộ được tài nguyên.');
      setMessage(data.message || `Đã đồng bộ ${data.imported || 0} tài nguyên.`);
      await loadOverview();
    } catch (err) {
      setError((err as Error).message);
    } finally {
  ...
  const stats = useMemo(() => ({
    ...
    pixel: assets.filter((asset) => asset.type === 'Dataset/Pixel').length,
  }), [assets, liveTokens.length, tokens.length]);

  const topTabs = topMount ? createPortal(
    <>
      <button data-resource-bridge="1" type="button" className={`${styles.topModeButton} ${mode === 'manage' ? styles.topModeActive : ''}`} onClick={() => openMode('manage')}>QUẢN LÝ TÀI NGUYÊN</button>
      <button data-resource-bridge="1" type="button" className={`${styles.topModeButton} ${mode === 'create' ? styles.topModeActive : ''}`} onClick={() => openMode('create')}>TẠO TÀI NGUYÊN</button>
    </>,
    topMount,
  ) : null;
  ...
  return (
    <>
      ...
      <div className={`main-wrap ${styles.compatMain} ${isLegacyTool ? styles.compatOpen : ''}`} aria-hidden={isLegacyTool ? undefined : true}>
        <header ref={headerHostRef} className={styles.hiddenTarget}><button type="button" data-bridge-anchor="1">Token</button></header>
      </div>

      {mode && (
        <div data-resource-bridge="1" className={styles.modeLayer}>
          <aside className={styles.modeSidebar}>
            <div className={styles.modeBrand}>
              <span><Zap size={19} /></span>
              <div><strong>{mode === 'manage' ? 'QUẢN LÝ TÀI NGUYÊN' : 'TẠO TÀI NGUYÊN'}</strong><small>ADS WORKSPACE</small></div>
              <button type="button" aria-label="Đóng" onClick={() => { setMode(null); setActiveTool(null); }}><X size={16} /></button>
            </div>
            <button type="button" className={`${styles.sideItem} ${activeTool === null ? styles.sideActive : ''}`} onClick={() => setActiveTool(null)}>
              <span><Boxes size={17} /></span><strong>Tổng quan</strong>
            </button>
            <div className={styles.sideLabel}>{mode === 'manage' ? 'VẬN HÀNH TÀI NGUYÊN' : 'LUỒNG TẠO'}</div>
            {modeTools.map((tool) => {
              const Icon = tool.icon;
              return (
                <button type="button" key={tool.key} className={`${styles.sideItem} ${activeTool === tool.key ? styles.sideActive : ''}`} onClick={() => openTool(tool)}>
                  <span><Icon size={17} /></span><strong>{tool.label}</strong><ChevronRight size={14} />
                </button>
              );
            })}
            <div className={styles.modeSidebarBottom}>
              <div><span className={liveTokens.length ? styles.onlineDot : styles.offlineDot} /><strong>{liveTokens.length}/{tokens.length} token LIVE</strong></div>
              <small>{assets.length} tài nguyên đã đồng bộ</small>
              <button type="button" onClick={() => openMode(mode === 'manage' ? 'create' : 'manage')}>{mode === 'manage' ? 'Chuyển sang Tạo tài nguyên' : 'Chuyển sang Quản lý tài nguyên'}</button>
            </div>
          </aside>
          ...
          {!activeTool && (
            <main className={styles.hubMain}>
              <div className={styles.hubHeader}>
                <div>
                  <span className={styles.eyebrow}>{mode === 'manage' ? 'RESOURCE OPERATIONS' : 'RESOURCE FACTORY'}</span>
                  <h1>{mode === 'manage' ? 'Quản lý tài nguyên' : 'Tạo tài nguyên'}</h1>
                  <p>{mode === 'manage'
                    ? 'Nạp token một lần, hệ thống check token rồi tự đồng bộ BM, TKQC, Page và Pixel. Các công cụ bên dưới dùng chung kho token này.'
                    : 'Tất cả luồng tạo hiện có được gom tại đây và dùng trực tiếp token đã nạp ở mục Quản lý tài nguyên.'}</p>
                </div>
                <button className={styles.secondaryButton} type="button" onClick={() => void loadOverview()} disabled={loadingOverview}>
                  {loadingOverview ? <LoaderCircle className={styles.spin} size={16} /> : <RefreshCw size={16} />} Làm mới
                </button>
              </div>
              ...
              <div className={styles.statsGrid}>
                <div><KeyRound /><span><small>Tổng token</small><strong>{stats.token}</strong></span></div>
                <div><ShieldCheck /><span><small>Token LIVE</small><strong>{stats.live}</strong></span></div>
                <div><Building2 /><span><small>Business Manager</small><strong>{stats.bm}</strong></span></div>
                <div><CreditCard /><span><small>TKQC</small><strong>{stats.ads}</strong></span></div>
                <div><Flag /><span><small>Page</small><strong>{stats.page}</strong></span></div>
                <div><Layers3 /><span><small>Pixel / Dataset</small><strong>{stats.pixel}</strong></span></div>
              </div>

              {mode === 'manage' ? (
                <section className={styles.quickStart}>
                  <div className={styles.quickStartHead}>
                    <div><span><KeyRound size={19} /></span><div><strong>Nạp token & chạy ngay</strong><small>Token được check trước; chỉ token LIVE mới được dùng để đồng bộ tài nguyên. Không tự retry khi Meta báo rate limit/lỗi.</small></div></div>
                    <button type="button" className={styles.primaryButton} onClick={() => void quickImportAndSync()} disabled={busy || !rawTokens.trim()}>{busy ? <LoaderCircle className={styles.spin} size={16} /> : <Zap size={16} />} Nạp token & đồng bộ</button>
                  </div>
                  <div className={styles.quickGrid}>
                    <label>Token
                      <textarea value={rawTokens} onChange={(event) => setRawTokens(event.target.value)} placeholder={'Mỗi dòng một token\nhoặc ten-token|token'} />
                    </label>
                    <div className={styles.quickSide}>
                      <label className={styles.fileBox}><FileUp size={21} /><strong>{fileName || 'Chọn file token'}</strong><small>TXT, CSV, TSV, JSON hoặc file text tối đa 5 MB</small><input type="file" onChange={(event) => void onTokenFile(event.target.files?.[0] || null)} /></label>
                      <label>Token đang dùng
                        <select value={selectedToken} onChange={(event) => setSelectedToken(event.target.value)}>
                          <option value="">Chọn token</option>
                          {tokens.map((token) => <option value={token.id} key={token.id}>{token.label} · {statusLabel[token.status]}</option>)}
                        </select>
                      </label>
                      <button className={styles.secondaryButton} type="button" onClick={() => void syncSelectedToken()} disabled={busy || !selectedToken}><RefreshCw size={15} /> Đồng bộ lại token đang chọn</button>
                    </div>
                  </div>
                </section>
              ) : (
                <section className={styles.createReady}>
                  <div><Building2 size={25} /><div><strong>Token nguồn cho luồng tạo</strong><small>Chọn token một lần. Công cụ tạo BM/hàng đợi sẽ đọc cùng kho token đã nạp.</small></div></div>
                  <select value={selectedToken} onChange={(event) => setSelectedToken(event.target.value)}>
                    <option value="">Chọn token</option>
                    {tokens.map((token) => <option value={token.id} key={token.id}>{token.label} · {statusLabel[token.status]}</option>)}
                  </select>
                  {!liveTokens.length && <button type="button" onClick={() => { setMode('manage'); setActiveTool('tokens'); }}>Chưa có token LIVE · Nạp token</button>}
                </section>
              )}
              ...
              <div className={styles.toolGrid}>
                {modeTools.map((tool) => {
                  const Icon = tool.icon;
                  return (
                    <button type="button" className={styles.toolCard} key={tool.key} onClick={() => openTool(tool)}>
                      <span className={styles.toolCardIcon}><Icon size={21} /></span>
                      <span><strong>{tool.label}</strong><small>{tool.description}</small></span>
                      <i>{tool.note || 'Sẵn sàng'}</i>
          ...
          {activeTool && activeInfo?.native && (
            <main className={styles.nativeMain}>
              <CampaignManager tokens={tokens} assets={assets} selectedToken={selectedToken} setSelectedToken={setSelectedToken} />
            </main>
          )}

          {activeTool && activeInfo && !activeInfo.native && (
            <>
              <div className={styles.toolTopBar}>
                <div><activeInfo.icon size={16} /><strong>{activeInfo.label}</strong></div>
                <button type="button" onClick={() => setActiveTool(null)}>Về tổng quan</button>
              </div>
              <Suspense fallback={<div className={styles.loading}>Đang nạp {activeInfo.label}…</div>}>
                <LegacyTool tool={activeTool} />
              </Suspense>
            </>
          )}
        </div>
